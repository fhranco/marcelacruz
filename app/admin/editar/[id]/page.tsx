...
export default function EditProduct() {
    ...
    const fetchProduct = async () => {
      ...
      if (product) {
        setFormData({ 
          ...
          galeria: product.galeria ? [...product.galeria, ...Array(4 - product.galeria.length).fill('')] : ['', '', '', ''],
          enStock: product.enStock ?? true
        });
      }
      setLoading(false);
    };
  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center">
      ...
      <span className="text-[10px] uppercase tracking-widest font-bold text-black/40">Cargando Atelier...</span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] flex flex-col p-8 md:p-16">
      <header className="max-w-7xl mx-auto w-full flex items-center justify-between mb-16 px-4">
        ...
        <h1 className="text-3xl md:text-4xl font-serif uppercase tracking-tighter">Editar Objeto Esencial</h1>
      </header>
        ...
        <div className="lg:col-span-2 flex flex-col gap-10">
          ...
          <div className="bg-white border border-black/5 p-10 flex items-center justify-between shadow-sm">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-[9px] text-black/30 font-bold uppercase">{formData.enStock ? 'Disponible' : 'Agotado'}</span>
            </div>
            <button 
              type="button" 
        ...
        <div className="lg:col-span-2 flex flex-col gap-10">
          <div className="bg-white border border-black/5 p-10 flex flex-col gap-10 shadow-sm">
            ...
            <div className="pt-10 border-t border-black/5">
              ...
              <div className="grid grid-cols-2 gap-8">
                 {formData.galeria.map((img: string, i: number) => (
                   <ImageUpload 
                    key={i}
                    label={`Detalle ${i + 1}`}
                    value={img}
