...
export default function ProductDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white pt-24 md:pt-48 pb-32 lg:pb-64">
      <div className="max-w-[1400px] mx-auto px-12 md:px-12 lg:px-24 flex flex-col lg:flex-row gap-16 md:gap-24 lg:gap-40">
        
        {/* LEFT: IMAGE GALLERY (Mobile First Stacked) */}
        <div className="w-full lg:w-3/5 flex flex-col gap-10 md:gap-20">
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 1.2 }}
            className="relative aspect-[4/5] bg-[#F5F5F5] flex items-center justify-center p-12 md:p-24 overflow-hidden"
          >
            ...
            {product.nuevo && (
              <div className="absolute top-8 left-8 px-6 py-3 bg-black text-white text-[10px] uppercase tracking-[0.4em] font-bold shadow-2xl">
                ESTRENO
              </div>
            )}
            {!product.enStock && (
              <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] flex items-center justify-center z-10">
                <span className="text-[11px] uppercase tracking-[0.6em] font-bold border-b border-black pb-2 text-black">AGOTADO</span>
              </div>
            )}
          </motion.div>
        ...
        <div className="w-full lg:w-2/5 flex flex-col lg:sticky lg:top-40 lg:h-fit">
          ...
          >
            ...
            <div className="flex flex-col gap-8 md:gap-10">
              <div className="flex items-center gap-6 md:gap-8 text-[11px] uppercase tracking-[0.3em] text-black/40 font-bold mb-4">
              ...
              <h1 className="text-4xl md:text-6xl lg:text-[100px] font-serif text-black uppercase tracking-tighter leading-[0.85] mb-8">
                {product.nombre}
              </h1>
              <div className="flex items-center gap-10 border-t border-black/5 pt-12 md:pt-16">
                <span className="text-4xl font-bold text-blackTracking-widest text-black">${product.precio.toLocaleString('es-CL')}</span>
                <div className="h-[0.5px] w-12 bg-black/20" />
            ...
            <div className="flex flex-col gap-10">
              <p className="text-black/70 leading-relaxed text-lg md:text-2xl font-light max-w-lg mb-8">
                {product.descripcion}
              </p>
              <div className="flex flex-col gap-10 py-12 md:py-16 border-y border-black/5">
            ...
            <div className="flex flex-col gap-16 md:gap-20">
              <div className="flex flex-col sm:flex-row items-center gap-8">
                <div className="w-full sm:w-auto flex items-center justify-between sm:justify-start border border-black/10 p-7 gap-12">
              ...
              {/* BENEFITS */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 text-[9px] uppercase tracking-[0.4em] font-bold text-black/30">
                <div className="flex items-center gap-8">
                  <Truck size={20} strokeWidth={0.5} />
                  <span>Envío Prioritario Chile</span>
                </div>
                <div className="flex items-center gap-8">
                  <ShieldCheck size={20} strokeWidth={0.5} />
                  <span>Garantía de Atelier</span>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      {/* RECOMENDADOS */}
      <section className="mt-40 md:mt-80 pt-32 md:pt-48 border-t border-black/5 w-full flex flex-col items-center">
         <span className="text-[11px] uppercase tracking-[0.6em] text-black/20 font-bold mb-20 px-8 text-center">Inspiraciones Complementarias</span>
         <div className="max-w-[1400px] w-full px-12 md:px-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-16 md:gap-32">
            {(product.id ? products.filter(p => p.id !== product.id).slice(0, 4) : []).map(p => (
              <div key={p.id} className="flex flex-col gap-10 text-center items-center cursor-pointer group" onClick={() => router.push(`/productos/${p.slug}`)}>
                <div className="aspect-[3/4] bg-[#F5F5F5] w-full flex items-center justify-center p-14 overflow-hidden relative">
                ...
                <div className="flex flex-col gap-6">
                  <span className="text-[13px] uppercase tracking-[0.4em] font-bold opacity-90">{p.nombre}</span>
                  <span className="text-[12px] text-black font-bold tracking-widest">${p.precio.toLocaleString('es-CL')}</span>
                </div>
              </div>
            ))}
         </div>
